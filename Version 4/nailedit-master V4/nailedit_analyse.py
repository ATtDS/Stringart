...
class Viewer(QtWidgets.QMainWindow):
    ...
    def __init__(self, parameters, scale=Point2(1.0,1.0)):
        ...
        self.multiWidget = QtWidgets.QScrollArea(self)
        self.multiWidget.setSizePolicy(QtWidgets.QSizePolicy.Expanding,
        ...
        self.imageLabel.setSizePolicy(QtWidgets.QSizePolicy.Expanding,
                QtWidgets.QSizePolicy.Expanding)
        ...
        self.imageLabel2.setSizePolicy(QtWidgets.QSizePolicy.Expanding,
        ...
        self.cutoffSlider = QtWidgets.QSlider(self.multiWidget, orientation=QtCore.Qt.Horizontal)
        ...
        self.deviation = []
        self.debug_cnt = 1
        self.minNailDist = 3.5 # keep this distance to the nails
    ...
    def openFile(self):
        ...
        filename = QtWidgets.QFileDialog.getOpenFileName(self, "Open Nailedit File", "./", "Nailedit (*.json)")[0]
        if filename:
    ...
    def saveNailsSVG(self):
        ...
        if "Nails" in self.parameters:
            filename = QtWidgets.QFileDialog.getSaveFileName(self, "Export Nails", "./", "SVG (*.svg)")[0]
            if filename:
    ...
    def generateGcode(self):
        ...
        filename = QtWidgets.QFileDialog.getSaveFileName(self, "Generate Gcode", "./", "gcode (*.tap)")[0]
        if not filename:
            return
...
def save_nails_SVG(nails, filename, scale):
    ...
    for p in pnts:
        svg.add(svg.circle((p[0]*ptmm*scale*svgwrite.mm, p[1]*ptmm*scale*svgwrite.mm), r*svgwrite.mm))
    svg.save()
...
def draw_nails(nails, img, showNails=True, lastString=10 ** 7, targetImage=None, reversed=False):
    ...
    stringCol[3] = 255
    width = oversampling
    print("buffer img %dx%d"%(iw,ih), "thread:", end=' ')
...
def draw_overlap(nails, img, lastString=10 ** 7, reversed=False):
    ...
    redlut = tuple(max(0, (i-85)*3) if i < 170 else 255 for i in range(256))
    greenlut = tuple(max(0, (i-170)*3) for i in range(256))
    bluelut = tuple( i*3 if i < 85 else max(0,255-i*3) for i in range(256) )
