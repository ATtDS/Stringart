...
class Mach3:
    ...
    def addStartup(self):
        ...
        ret = """
            G90 G94 G91.1 G40 G49 G17 F1000
            G1 F1000
            G21
    ...
    def cannedDrillCycle(self, points, startz, depthz, feed):
        ...
        p = points[0]
        self.program.append("G98 G81 X{x:.3f} Y{y:.3f} Z{z:.3f} R{r:.3f} F{f:.1f}".format(x=p.y, y=p.x, z=depthz, r=startz, f=feed))
        for p in points[1:]:
            self.program.append("X{x:.3f} Y{y:.3f}".format(x=p.y, y=p.x))
    ...
    def arcTo(self, x, y, z, pivotx, pivoty, ccw=False):
        ...
        #TODO ändrade till 0 från 0.5
        if Point2(x,y).dist(Point2(self.mposX, self.mposY)) < 0: # straight move instead
            mv = 'G1 Y{x:.3f} X{y:.3f}'.format(x=x, y=y)
        else:
            mv = 'G{move} Y{x:.3f} X{y:.3f} J{i:.3f} I{j:.3f}'.format(move=2 if ccw else 3, x=x, y=y, i=pivotx-self.mposX, j=pivoty-self.mposY)
        self.mposX = x
        self.mposY = y
...
#TODO i have changed this from 0.5 to 0 
            if len(intersects) == 2 and intersects[0].dist(intersects[1]) > 0:
