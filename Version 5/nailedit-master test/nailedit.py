...
class Viewer(QtWidgets.QMainWindow):
    ...
    def __init__(self, parameters):
        ...
        self.imgCounter = 0
        self.outPath = "render/img_{:04d}.jpg"
    ...
    def save_as(self, filename):
        ...
        data = { "1:summary": {
                 ...
                 "2:parameters:" : dict(kv for kv in self.parameters.items() if is_jsonable(kv[1])),
    ...
    def closeEvent(self, event):
        ...
        counts = [(c[1], c[0]) for c in self.segmentCount.items()]
        ...
        if self.mode == "Threading":
            msg = QtWidgets.QMessageBox.question(self, "Quit", "Save it?", QtWidgets.QMessageBox.Yes|QtWidgets.QMessageBox.No|QtWidgets.QMessageBox.Cancel)
            if msg == QtWidgets.QMessageBox.Yes:
                filename = QtWidgets.QFileDialog.getSaveFileName(self, "Save as", "./", "Nailedit (*.json)")
                if filename:
    ...
    def workPoints(self):
        ...
        if "PointCloud" in self.parameters:
            ...
            for i in range(0, numy, 2):  # mask grid rim
                linestart = int(gridstart + i*numx - (i/2))
                pc.p[linestart].heat = 1
                pc.p[linestart + (numx-1)].heat = 1
            for i in range(numx):
                pc.p[gridstart+i].heat = 1
                pc.p[int(gridstart + i + (numy-1)*numx-((numy-1)/2))].heat = 1
    ...
    def March(self):
        ...
        pnts.cool(0.1)
        pnts.p[bestMatch[1]].heat = 1


        print("iteration", self.iterationCounter, "residual", bestMatch[2], "improvement", improvement, "avg", self.avg_improvement, end=' ')
...
def sel_blur(img_np, mask_np):
    ...
    for m in range(num_mipmaps):
        #f = 2**(m+1)
        f = (2 + m)
        mm = img.resize( (int(img.width/f), int(img.height/f)), resample=Image.BICUBIC ).resize( img.size, resample=Image.BICUBIC )
...
if __name__ == '__main__':
    ...
    # Define default values for any potentially missing parameters
    default_params = {
        "proc_width": 600,
        "proc_height": 600,
        "ppi": 0.3 / 600,  # meters per pixel
        "searchRadius": 0.25,
        "nailDistMin": 9 / 1000.0 / (0.3 / 600),  # minimum nail distance
        "nailDistMax": 20.0 / 1000.0 / (0.3 / 600),  # maximum nail distance
        "nailDiameter": 1.5 / 1000.0 / (0.3 / 600),  # diameter of the nail
        "backgroundColor": 0,  # canvas color
        "threadColor": (255, 160),  # string color
        "currentPoint": 0,
        "lastPoint": -1,
        "start_at": (0.5, 0),  # closest point to start in relative coordinates [0..1]
        "inputImagePath": "flower.png",
    ...
    # Update the parameters dictionary with defaults for any missing elements
    for key, value in default_params.items():
        params.setdefault(key, value)

    # Load and enhance the image based on the parameters
    img = Image.open(params["inputImagePath"]).convert("RGB")
    params["proc_width"] = int(params["proc_height"] * float(img.width) / img.height)
    print(f"Input image {img.width}x{img.height}, proc dim: {params['proc_width']}x{params['proc_height']}")
    img = Enhance(img, params)
    params["TargetImage"] = img

    # Instantiate and show the viewer with the parameters
